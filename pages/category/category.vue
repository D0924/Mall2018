 ...
 * @LastEditTime: 2022-05-26 22:17:06
...
<template>
  <view class="scroll-view-wrap">
    <scroll-view class="left" :style="{ height: windowHeight + 'px' }" scroll-y>
      <view
        :class="['left-scroll-view', index === active ? 'active' : '']"
        @click="handlerChangeActive(index)"
      ...
      <view v-for="(item, index) in categoryListLevel2" :key="index">
        ...
        <view class="wrap">
          <view
            ...
            :key="index"
            @click="handlerCategoryLevel3Click(item.cat_id)"
          >
    ...
    handlerChangeActive(index) {
      ...
      this.scorllTop = Math.random();
		...
		handlerCategoryLevel3Click(id){
			uni.navigateTo({ url: `/subPackage/goodsList/goodsList?id=${id}` })
		}
  },
};
</script>
